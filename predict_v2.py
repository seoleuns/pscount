...
def predict_and_visualize(model_path, image_dir, output_dir, save_images=True):
    ...
    for tif_file in tif_files:
        ...
        _mask = img > 0
        if _mask.any():
            img[_mask] = np.clip(img[_mask] * 3, 0, 255).astype(np.uint8)
        if len(img.shape) == 3:
           img_input = img[:,:,1]  
        else:
           img_input = img
        ...
        if model_path == "default":
           masks, flows, _ = model.eval(img_input.astype(np.float32) / 255.0, cellprob_threshold=-4, flow_threshold=1.5, normalize=False)
        else:
           masks, flows, _ = model.eval(img_input.astype(np.float32) / 255.0, cellprob_threshold=-4, flow_threshold=1.5, normalize=False)
    ...
    print(f"results 개수: {len(results)}")
...
def evaluate_with_ground_truth(model_path, image_dir, labeled_dir, output_dir, save_images=True):
    ...
    for labeled_path in labeled_files:
        labeled_stem = labeled_path.stem      # Excluding the extension
        labeled_ext = labeled_path.suffix     # .tif

        match = re.search(r'_(\d+)count$', labeled_stem)
        if not match:
            continue
        true_count = int(match.group(1))

        original_stem = re.sub(r'_\d+count$', '', labeled_stem)
        original_name = original_stem + labeled_ext
